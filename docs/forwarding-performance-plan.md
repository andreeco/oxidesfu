...
| Partial | Differential evidence | The paired profiler emits a versioned Rust SDK client-observed artifact for every Go and Oxide run and now retains Oxide forwarding JSONL for target/selection/PLI/driver correlation. The first real sweep stopped at empty Go `perf.data` post-processing; this is now non-fatal so the required real sweep can retain media evidence even if CPU profiling is unavailable. Rerun and compare the resulting post-warm-up artifacts. |
