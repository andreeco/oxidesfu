...
Validation: RTC 38 tests passed; signaling 534 passed with 3 ignored; `cargo test --workspace` now passes with 119 passed and 9 ignored (including native VP9 descriptor low→high); fresh Firefox receiver suite 3/3 passed including VP9 SVC; focused descriptor rewrite/retransmission tests passed. Strict Clippy passes for the RTC, signaling, and server crates. Remaining work is repeated performance evidence, published remote repin of the temporary local rust-sdks fix, and surgical `oxidesfu-test` test-support lint cleanup—not a known forwarding-path correctness gap.
