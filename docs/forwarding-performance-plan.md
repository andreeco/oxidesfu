...
| Maintenance pending | Workspace hygiene | Strict Clippy passes for `oxidesfu-rtc`; workspace Clippy still stops in the existing signaling lint backlog. A current workspace test run has one reproducible signal-only aggregate-probe mismatch: it expects demand before a room media subscription is active. Resolve those in separate maintenance slices rather than changing forwarding behavior. |
