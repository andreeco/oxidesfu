...
**Status: complete for current spatial acquisition behavior.** The reader's 250 ms timer drives bounded target-local selector PLIs; downstream PLI/FIR relay remains separate. VP9/AV1 source transitions use dependency-descriptor frame-start plus active DTI `Switch` when metadata is present, with codec parsing only when descriptors are absent. Real scalable packet-sequence coverage is complete; native SDK fixture promotion awaits a valid deterministic layered VP9/AV1 access-unit corpus, not a descriptor-injection API.
