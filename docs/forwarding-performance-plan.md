...
| Blocked on fork publication | Dependency-descriptor switching | Local RTC commit `56a36e408913475baeeb5672bd3e30036dea820f` adds descriptor packet metadata that requires both a frame start and an active DTI `Switch` target; its nine parser regressions pass. Published outer WebRTC `9f49b55260836d7aaff6768ac14acd5557600dcc` carries only nested RTC `bd54faa2a648a88c9434f095130e4a5f0b078a8e`, which lacks the required `Switch` semantic. Publish the nested successor and a matching outer commit, then advance Oxide's Git pin and add VP9/AV1 scalable-stream switching contracts. SSH publication is presently blocked by transient DNS resolution failure for `github.com`; do not pin the incomplete published API. |
