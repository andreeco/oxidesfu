...
| Maintenance pending | Workspace hygiene | Strict Clippy passes for `oxidesfu-rtc`; workspace Clippy still stops in the existing signaling lint backlog. The signal-only aggregate-probe mismatch is resolved: quality demand is now suppressed until a compatible media receiver is active, matching upstream `SubscribedTrack.OnCodecNegotiated`; genuine unsubscribe/leave still emits all-off demand. |
