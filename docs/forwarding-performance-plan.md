...
| Partial | Differential evidence | The paired profiler now emits a versioned Rust SDK client-observed post-warm-up artifact for every Go and Oxide run: stable track identities, received bytes/sec, decoded dimensions/frames, drops/discards, PLI/NACK counts, and dimension transitions. It explicitly reports server backpressure as unavailable rather than inferring it. Run a real paired sweep, then add an Oxide machine-readable server snapshot for target/selection/PLI/driver correlation if client evidence alone is insufficient. |
