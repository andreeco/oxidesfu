...
| Partial | Differential evidence | The paired profiler emits a versioned Rust SDK client-observed artifact for every Go and Oxide run and retains Oxide forwarding JSONL for target/selection/PLI/driver correlation. The one-run 30-second sweep at `target/profiles/paired-mixed_room_high_simulcast_large-20260716T074243Z-2555c723` retained all artifacts despite empty CPU profiles. Go video observers received about `1.00–1.01 MB/s` at `1280×720`; Oxide received about `0.21–0.46 MB/s` with mixed `320×150`/`320×180`/`1280×720`. Audio-only remained matched. Correlate Oxide snapshot state, repair the remaining low-layer convergence, then repeat with multiple runs. |
