...
Validation: RTC 38 tests passed; signaling 534 passed with 3 ignored; `cargo test --workspace` passed with 118 passed and 10 ignored; fresh Firefox receiver suite 3/3 passed including VP9 SVC; focused descriptor rewrite/retransmission tests passed. Strict Clippy passes for the RTC, signaling, and server crates. Remaining work is repeated performance evidence, deterministic native SDK scalable-fixture coverage, and surgical `oxidesfu-test` test-support lint cleanup—not a known forwarding-path correctness gap.
...
| Complete | Dependency-descriptor switching and browser scalable-codec forwarding | Oxide is pinned to published outer WebRTC `2133ab09ae3681872b7a98773bd56d682056ed87`, nested RTC `69571fe`. `RemoteTrackState` processes real stateful descriptor sequences; each target selects frames with direct/chain dependency protection, rewrites the active decode-target mask at the subscriber-negotiated extension ID, strips unnegotiated descriptors, and caches the final rewritten packet for NACK. Runtime primary-RTP codec reconciliation negotiates VP9 profile 0 (`PT 98`) rather than silently defaulting the sender to VP8. One-SSRC VP9/AV1 sources remain distinct from simulcast SSRC selection. RTC regressions and the fresh-server Firefox VP9 SVC contract pass. Native probes pin Rust SDK `9afe85bb8593c9e955de4ee4949706fc04699ed9`; checked-in reproducible VP9 low/high keyframes and valid descriptors are accepted by its pre-encoded source, but currently yield no decoded remote frame. Promote the ignored native contract after that SDK/libwebrtc descriptor delivery defect is repaired. |
...
| Evidence complete for two retained rounds | Differential media parity | The paired profiler emits versioned Rust SDK client evidence and Oxide forwarding JSONL. The retained 30-second sweeps at `20260716T075350Z-228c19ba` and `20260716T130224Z-ff0beac2` both show comparable high video: in the latter all four observer video tracks are `1280×720`, Go delivers `1,000,805–1,003,199 B/s`, and Oxide `974,209–1,001,683 B/s`; audio-only is `10,267` vs `10,301 B/s`. Repeat idle-host rounds remain required for CPU conclusions. |
| Maintenance pending | Workspace hygiene | Strict Clippy passes for `oxidesfu-rtc`, `oxidesfu-signaling`, and `oxidesfu-server`. Workspace Clippy now reaches only `oxidesfu-test` test-support hygiene: legacy protocol fixture fields, dead opt-in harness helpers, and style lints. The signal-only aggregate-probe mismatch is resolved: quality demand is now suppressed until a compatible media receiver is active, matching upstream `SubscribedTrack.OnCodecNegotiated`; genuine unsubscribe/leave still emits all-off demand. Do this remaining test cleanup as surgical per-file changes: a bulk formatter attempt produced more than 12,000 unrelated diff lines and was discarded. |
...
**Status: complete for current spatial acquisition behavior.** The reader's 250 ms timer drives bounded target-local selector PLIs; downstream PLI/FIR relay remains separate. VP9/AV1 source transitions use dependency-descriptor frame-start plus active DTI `Switch` when metadata is present, with codec parsing only when descriptors are absent. Real scalable packet-sequence coverage is complete; native SDK fixture media and descriptor injection are now present, while promotion awaits repair of the SDK/libwebrtc pre-encoded descriptor delivery path.
